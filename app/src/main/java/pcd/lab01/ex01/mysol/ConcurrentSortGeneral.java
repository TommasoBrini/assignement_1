...
import pcd.lab01.ex01.sol.MergingWorkerGeneral;
import pcd.lab01.ex01.sol.SortingWorker;
...
public class ConcurrentSortGeneral {
    ...
    public static void main(String[] args) {
        ...
        List<MySortedThread> workers = new ArrayList<>();
        for (int i = 0; i < nAgents - 1; i++) {
            var w = new MySortedThread("worker-"+(i+1), v, from, to);
            w.start();
            workers.add(w);
            from = to + 1;
            to += jobSize;
        }
        var w = new MySortedThread("worker-"+nAgents, v, from, v.length - 1);
        w.start();
        workers.add(w);

        MyGeneralMergedThread m = new MyGeneralMergedThread("merger", v, workers);
        m.start();
        try {
            m.join();
